<?php  
    ...
    // получить массив всех файлов с картинками
    function getImages()
    {               
        return array_diff(scandir(UPLOADS_DIR), array('..', '.'));
    }        
                    ...
                    <?php
                        ...
                        if (!empty($images))
                            foreach ($images as $image) {
                                $thumbnail = THUMBNAILS_DIR . $image;
                                $original = UPLOADS_DIR . $image;
                                $width = THUMB_WIDTH;
                                echo <<<HTML
                                <div class="image">
                                    <a href="$original" target="_blank"><img src="$thumbnail" alt="$image" width=$width></a>
                                </div>
HTML;
                            }
                    ?>
